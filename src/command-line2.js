...
program
  ...
  .option(
    "-f <name>",
    'Write tags to specified file. Value of "-" writes tags ' +
      'to stdout ["tags"; or "TAGS" when -e supplied].',
    "tags",
  )
  .option(
    "--extension <extension>",
    "File extension to look for. Default is `.js`",
    "js",
  )
  .action((file, others) => {
    // Filtering
      ...
      if (fs.statSync(item).isDirectory()) {
        if (program.recurse) {
          diveSync(
            item,
            {
              recursive: program.recurse,
              filter: (filepath, dir) => !filter(filepath, dir),
            },
            (err, filepath) => {
              files.push(filepath);
            },
    ...
    // files = [
    //   // "/Users/shawn.axsom/dev/ambyint/packages/web/src/routes/well/tabs/setup/well-reservoir-information-dialog/well-reservoir-information-dialog.js",
    //   // "/Users/shawn.axsom/dev/ambyint/packages/web/src/routes/well/tabs/setup/save-cancel-buttons.js",
    //   "/Users/shawn.axsom/dev/ambyint/packages/web/src/actions/wells.js",
    // ];

    // console.log("files", files)
    // Process each file
    // async.map(files, (filepath, callback) => {
    condense(files, (error, results) => {
      ...
      async.map(
        ...
        (filepath, callback) => {
          ...
          const result = results.filter(
            r => r["!name"] === path.basename(filepath),
          );
